 ...
 * <p>
 * 예를 들어, ababcdcdababcdcd의 경우 문자를 1개 단위로 자르면 전혀 압축되지 않지만, 2개 단위로 잘라서 압축한다면 2ab2cd2ab2cd로 표현할 수 있습니다. 다른 방법으로 8개 단위로 잘라서 압축한다면 2ababcdcd로 표현할 수 있으며,
...
public class Day200910 {
    ...
    public static int solution(String s) {
        ...
        while (seperLen <= ((s.length() / 2) + 1)) {

            int totalLen = seperLen;
            String tmps = s.substring(0, seperLen);
            int eqCount = 1;
            for (int i = seperLen; i <= s.length() - seperLen; i += seperLen) {
                String substring = s.substring(i, i + seperLen);
                if (!tmps.equals(substring)) {
                    System.out.println(eqCount + tmps);
                    tmps = substring;
                    totalLen = eqCount > 1 ? totalLen + 1 : totalLen;
                    totalLen += seperLen;
                    eqCount = 1;
                } else {
                    eqCount++;
            ...
            totalLen = eqCount > 1 ? totalLen + 1 : totalLen;
            totalLen = s.length() % seperLen > 0 ? totalLen + s.length() % seperLen : totalLen;
